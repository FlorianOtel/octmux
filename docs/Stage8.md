...
**Commit(s):** `e25b2f9` (src), pending docs commit

Hard-evidence investigation against the local OpenCode source at `~/Gin-AI/projects/opencode/` (including local build) proved that the OC daemon does NOT emit `message.part.updated` with `part.type === "subtask"` for Task-tool dispatches — by design, not by upstream bug. `packages/opencode/src/tool/task.ts:148` shows the Task tool's entire dispatch is `yield* sessions.create({ parentID, agent, model, ... })` with zero part-append, zero `Bus.publish`, zero SubtaskPart construction. `packages/opencode/src/session/session.ts:577` publishes `session.created` on the global event stream with `info.parentID`, `info.agent`, `info.model` set on child sessions (agent/model restored upstream in FlorianOtel/opencode@98a4907c9, Stage 8.1.3). `packages/opencode/src/session/prompt.ts:1594` is the only `SubtaskPart`-constructing site and sends it as a USER MESSAGE PART (`SubtaskPartInput`) for the `/command subtask: true` flow — unrelated to Task-tool dispatch.

Stage 8.1.1 (`7256de8`) and Stage 8.1.4 (`eb3a777`) both built on a misread of the OC protocol. Stage 8.1.4's queue-and-drain solved a race that could only matter if the upstream SubtaskPart event arrived — which it doesn't. This stage removes the dead code path entirely: in `src/events.ts` the SubtaskPart filter branch (lines 269-282), `detectedSubtaskPartIDs` Set + every reference, and the subtask-end emission loops in `synthesizeSessionIdleEvents` / `session.idle` / `message.part.removed`; in `src/orchestra-watch.ts` `_subagentPollers`, `_startSubagentPoll`, `_bumpSubagentActivity`, `_stopSubagentPoll`, the `dispose()` poller-cleanup loop, `agentModelMap`, `loadAgentModels()`, the `sessionID?` parameter on the notifier, and the per-subagent `partID` / `modelRaw` / `modelLabel` / `sessionID?` fields. New mechanism: `events.ts` adds `trackedChildSessions: Set<string>` and a `session.created` branch that filters by `info.parentID === sessionID` and emits a `subagent-detected` ReplEvent with `sessionID`, `agent`, `model` (the latter formatted as `${providerID}/${id}` from the SDK's structured `SessionModel`). A `session.deleted` branch and a tracked-child branch in `session.idle` emit `subagent-ended`. A tracked-child branch in `session.updated` emits a new `subagent-activity` ReplEvent that drives per-row spinner-vs-frozen state, replacing the dormant D-poll. `orchestra-watch.ts` renames the notifier methods to `notifySubagentStarted/Ended/AllSubagentsEnded`, keys `badge.subagents[]` by `sessionID`, adds a `model: string` field, and ships a `notifySubagentActivity(sessionID, ts)` method mirroring `notifyParentActivity`. The three mutation methods (`Started`, `Ended`, `AllEnded`) now emit `"changed"` directly — `_updateBadge`'s `JSON.stringify` diff cannot detect in-place `subagents[]` mutations after a shallow spread (the array reference is shared), a latent bug carried over from 8.1.4 that would have prevented row rendering even if the upstream SubtaskPart signal had arrived. `StatusLine.tsx` adds a dim bracketed `[provider/model]` segment (e.g. `◐ planner [sohoai/minimax-m3]`); spinner glyph, `#1dde00` colour, 120s freeze threshold, and downward stack direction are preserved from Stage 8.1.4. New `bun:test` files at `src/events.test.ts` and `src/orchestra-watch.test.ts` (48 assertions, all green) pin the new detection-layer behaviour; `package.json` gains a `"test": "bun test src/"` script. Dead-code grep gate (`SubtaskPart|notifySubtask|_subagentPoll|_pendingSubtaskQueue|detectedSubtaskPartIDs|D-poll` in `src/`) returns zero substantive hits.
