...
● Now I can give a definitive answer. The current code has eager pane creation in setup() (the original pre-Phase 4.3 code) — panes are created before any events flow, SIGWINCH is handled, and tail -f is already running. The FIFO write path is identical in both renderers. 
