...
**Note (2026-06-12, Stage 7 fix):** `marginBottom` on the outer status-area Box was subsequently reduced from `{2}` to `{0}` to bring the visible status area from 6 lines to 4. Ink itself contributes 1 trailing blank line below the root element regardless of `marginBottom` setting. The `CHROME_ROWS = 10` budget here was replaced by measured chrome in Stage 11.1 (`restRows` live measurement), so this decomposition is historical. The Ink trailing-newline behaviour means `marginBottom={0}` still produces 1 blank line at the bottom of the status area; to target N blank lines set `marginBottom={N-1}`.
...
**Multi-line `PromptInput` and modal overlays mutate chrome height.** Single-line input + single-line status + horizontal rule + marginBottom={2} totals 5 rows (historical — `marginBottom` was later reduced to `{0}`; Ink contributes 1 trailing blank regardless, so effective idle chrome is now 4 rows); multi-line input or any modal adds 2–5 more. With a tight `CHROME_ROWS = 6` reserve, a 1-row chrome bump was enough to cross the overflow ceiling. The `CHROME_ROWS = 10` reserve absorbs chrome transients up to 4 extra rows above the typical 5 — safe for the typical multi-line-input + small-modal case.
